...
public class Filter 
{
	...
	/**
	 * tolerance 0-100 in how much % the compared pixel can deviate from the set color.
	 */
	...
	public boolean match(BufferedImage i)
	{
		...
		for(int cpoint=0; cpoint<points.length;cpoint++) {
			if (points[cpoint][0] >= i.getWidth() || points[cpoint][1] >= i.getHeight()) {
				...
				// Test R G B
//				System.out.println("R diff: " + Math.round(100*(Math.abs(points[cpoint][2] - c.getRed())/255.0)));
//				System.out.println("B diff: " + Math.round(100*(Math.abs(points[cpoint][3] - c.getGreen())/255.0)));
//				System.out.println("G diff: " + Math.round(100*(Math.abs(points[cpoint][4] - c.getBlue())/255.0)));
