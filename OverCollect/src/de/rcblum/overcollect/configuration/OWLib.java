...
public class OWLib 
{
	...
	private void init() 
	{
		...
		for (File res : resolutionFolders) {
			if (res.isDirectory()) {
				...
				if (dimensionStrings.length == 2 && dimensionStrings[0].matches("\\d+") && dimensionStrings[1].matches("\\d+")) {
	...
	{
			return this.items.get("1920x1080").values().stream().filter(i -> i.isMap()).map(i -> i.getItemName()).collect(Collectors.toList());
	}

	public List<String> getMatchIndicators() 
	{
			return this.items.get("1920x1080").values().stream().filter(i -> i.isMatchIndicator()).map(i -> i.getItemName()).collect(Collectors.toList());
	}
	...
	{
		return this.items.get("ocr_primary_font") != null ? this.items.get("ocr_primary_font").values().stream().filter(i -> i.hasGlyph()).map(i -> i.getGlyph()).collect(Collectors.toList()) : null;
	}
	
	public List<Glyph> getSecondaryFontGlyphs()
	{
		return this.items.get("ocr_secondary_font") != null ? this.items.get("ocr_secondary_font").values().stream().filter(i -> i.hasGlyph()).map(i -> i.getGlyph()).collect(Collectors.toList()) : null;
	}
	...
	{
		return this.items.get(width+"x"+height) != null ? this.items.get(width+"x"+height).values().stream().filter(i -> i.drop()).collect(Collectors.toList()) : new LinkedList<>();
	}
}
