	...
	public ScreenExtract(BufferedImage endScreen, OCRConfiguration config) 
	{
		...
		if (this.config.values != null) {
			for (String field : this.config.values.keySet()) {
				...
				System.out.println("Primary Field: [name=" + field + ", x="+ bounds[0] + ", y=" + bounds[1] + ", w=" + this.config.secondaryDataFieldSize[0] + ", h=" + this.config.secondaryDataFieldSize[1] + "]");
				this.valueImages.put(field, adjustImage(this.screenImg.getSubimage(bounds[0], bounds[1],
						this.config.dataFieldSize[0], this.config.dataFieldSize[1]), this.config.getDataColor(), config.skew, config.skewTrim,
						config.doRecolor));
			}
		}
		if (this.config.secondaryValues != null) {
			for (String field : this.config.secondaryValues.keySet()) {
				...
				System.out.println("Secondary Field: [name=" + field + ", x="+ bounds[0] + ", y=" + bounds[1] + ", w=" + this.config.secondaryDataFieldSize[0] + ", h=" + this.config.secondaryDataFieldSize[1] + "]");
				this.secondaryValueImages.put(field, this.screenImg.getSubimage(bounds[0], bounds[1], this.config.secondaryDataFieldSize[0], this.config.secondaryDataFieldSize[1]));
			}
		}
		readValues();
	}
...
//				try {
//					ImageIO.write(img, "PNG", Paths.get("tmp", field + ".png").toFile());
//				} catch (IOException e) {
...
//				}
				...
				for (int i = 0; i < charImgs.length; i++) {
					for (Glyph glyph : glyphs) {
						int newW = Math.round(charImgs[i].getWidth() * (((float)glyph.getBaseFontSize())/((float)charImgs[i].getHeight())));
						int newH = Math.round(charImgs[i].getHeight() * (((float)glyph.getBaseFontSize())/((float)charImgs[i].getHeight())));
						BufferedImage rescaled = glyph.getBaseFontSize() != charImgs[i].getHeight() ? resize(charImgs[i], newW, newH) : charImgs[i];
						if (OWLib.getInstance().getBoolean("debug")) {
							try {
								ImageIO.write(rescaled, "PNG", Paths.get("tmp",  field + "[" + i + "]_rescaled.png").toFile());
							} catch (IOException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
						...
						if (glyph.match(rescaled, this.config.getDataColor(), 0.06f)) {
							try {
								ImageIO.write(rescaled, "PNG", Paths.get("tmp",  (glyph.getChar() == ':' ? '_' : glyph.getChar()) + "_" + field + "[" + i + "]_rescaled.png").toFile());
							} catch (IOException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
					...
					if (chars[i] == ' ') {
						try {
							...
							ImageIO.write(charImgs[i], "PNG", Paths.get("tmp",  "NOHIT_" + field + "[" + i + "]_rescaled.png").toFile());
						} catch (IOException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
...
//				}
				...
				imgResc = adjustImage(imgResc, this.config.getSecondaryDataColor(),
						config.skewSecondary, config.skewSecondaryTrim, config.doRecolor);
				...
				for (int i = 0; i < charImgs.length; i++) {
					for (Glyph glyph : glyphs) {
//						int newW = (int)(charImgs[i].getWidth() * glyph.getBaseFontSize()/(float)config.secondaryFontSize);
//						int newH = (int)(charImgs[i].getHeight() * glyph.getBaseFontSize()/(float)config.secondaryFontSize);
						BufferedImage rescaled = charImgs[i]; // glyph.getBaseFontSize() != config.secondaryFontSize? resize(charImgs[i], newW, newH) : charImgs[i];
//						try {
//							ImageIO.write(rescaled, "PNG", Paths.get("tmp", field + "[" + i + "].png").toFile());
//						} catch (IOException e) {
//							// TODO Auto-generated catch block
//							e.printStackTrace();
//						}
						if (glyph.match(rescaled, this.config.getSecondaryDataColor(), 0.06f))
							chars[i] = chars[i] != ' ' && chars[i] == '3' && glyph.getChar() == '8' ? chars[i] : glyph.getChar();
					}
					if (chars[i] == ' ') {
						try {
							this.nohitSecondary.put(charImgs[i], mostProbableGlyph[i]);
							ImageIO.write(charImgs[i], "PNG", Paths.get("tmp",  "NOHIT_" + field + "[" + i + "]_secondary_rescaled.png").toFile());
						} catch (IOException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
...
//				try {
//					
//					this.secondaryValues.put(field, instance.doOCR(img).trim().replace(",", ""));
...
//				}
	...
	public static BufferedImage adjustImage(BufferedImage source, Color primaryColor, double skewX, int skewTrim,  boolean doRecolor)
	{
		if (doRecolor) {
			// Reverse color
			...
			for (int x = 0; x < source.getWidth(); x++) {
				for (int y = 0; y < source.getHeight(); y++) {
					...
					Color c = null;
					int tolerance = 80;
					if (Math.abs(pixelR-textR)<tolerance && Math.abs(pixelG-textG)<tolerance && Math.abs(pixelB-textB)<tolerance)
						c = primaryColor;
					else
						c = new Color(255-textR, 255-textG, 255-textB);
					
//					int colorValue = (pixelR - 16 < 15 && pixelG - 205 < 12 && pixelB - 22 < 10) ? 203 + 205 + 208
//							: (col.getRed() + col.getGreen() + col.getBlue());
//					colorValue = colorValue <= 200 ? colorValue / 15 : colorValue / 2;
//					colorValue = colorValue > 255 ? 255 : colorValue;
//					col = new Color(255 - colorValue, 255 - colorValue, 255 - colorValue);
