...
public class JFilterSetup extends JFrame {
	...
	public JFilterSetup() {
		...
		chckbxIsMatchindicator.setToolTipText("The screenshot can be used to detect the beginning of a new competitive match");
		...
		chckbxIsGlyph.setToolTipText("Indicates that the filter can be used to create a glyph, which extracts Data from screenshots");
		...
		chckbxDropScreenshot.setToolTipText("Every Screenshot this matches will be dropped before processing it further");
...
//						lblImage.setMinimumSize(new Dimension(image.getWidth(), image.getHeight()));
					...
					if (item.hasFilter()) {
						...
						for(int i=0; i<filter.points.length; i++) {
							model.addRow(new Object[]{filter.points[i][0], filter.points[i][1],
									filter.points[i][2], filter.points[i][3], filter.points[i][4]});
						}
					}
